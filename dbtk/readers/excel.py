...
class XLSXReader(Reader):
    ...
    def _read_headers(self) -> List[str]:
        ...
        if not self._headers_read:
            # openpyxl is 1-based, so row 1 is index 0
    ...
    def _generate_rows(self) -> Iterator[List[Any]]:
        """Yield data rows from the Excel worksheet starting from row 1 (after header).

        Yields:
            List of cell values for each data row.
        """
        start_row = 1 if self._raw_headers is not None else 2  # Start after header if headers provided
        for row_num in range(start_row, self.ws.max_row + 1):  # 1-based
...
class XLReader(Reader):
    ...
    def _generate_rows(self) -> Iterator[List[Any]]:
        ...
        """
        start_row = 0 if self._raw_headers is not None else 1  # Start after header if headers provided
        for row_num in range(start_row, self.ws.nrows):  # 0-based
